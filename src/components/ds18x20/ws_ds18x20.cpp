...
*/
/********************************************************************/
bool ws_ds18x20::addDS18x20(wippersnapper_ds18x20_Ds18x20Add *msgAdd) {
  bool is_success = false;

  // init. new ds18x20 object
  ds18x20Obj *newObj = new ds18x20Obj();
  char *oneWirePin = msgAdd->onewire_pin + 1;
  ...
  if (newObj->dallasTempObj->getAddress(newObj->dallasTempAddr, 0)) {
    ...
    newObj->dallasTempObj->setResolution(msgAdd->sensor_resolution);
    // copy the device's sensor properties
    newObj->sensorPropertiesCount = msgAdd->sensor_properties_count;
    // TODO: Make sure this works, it's a new idea and untested :)
    for (int i = 0; i < newObj->sensorPropertiesCount; i++) {
      newObj->sensorProperties[i].type =
          msgAdd->sensor_properties[i].sensor_type;
      newObj->sensorProperties[i].sensor_period =
          (long)msgAdd->sensor_properties[i].sensor_period * 1000;
    }
    // set pin
    strcpy(newObj->onewire_pin, msgAdd->onewire_pin);
  ...
  // TODO: putback packet encoding and publishing here
  ...
  wippersnapper_signal_v1_Ds18x20Response_fields, &msgInitResp)) {
...
/*!
    @brief    De-initializes a DS18x20 sensor and releases its
              pin and resources.
    @param    msgRemove
              Message containing configuration data about a
              ds18x20 sensor.
*/
/********************************************************************/
void ws_ds18x20::deleteDS18x20(wippersnapper_ds18x20_Ds18x20Remove *msgRemove) {
  // Loop thru vector of drivers to find the unique address
  for (size_t idx = 0; idx < _ds18xDrivers.size(); idx++) {
    if (strcmp(_ds18xDrivers[idx]->onewire_pin, msgRemove->onewire_pin) == 0) {
      WS_DEBUG_PRINT("Deleting OneWire instance on pin ");
      WS_DEBUG_PRINTLN(msgRemove->onewire_pin);
...
void ws_ds18x20::update() {
  ...
  for (iter = _ds18xDrivers.begin(), end = _ds18xDrivers.end(); iter != end;
    ...
    // TODO: putback after initial refactor
    ...
    /*     // Create an empty DS18x20 event signal message and configure
        ...
        for (int i = 0; i < (*iter)->sensorPropertiesCount; i++) {
          ...
          if (curTime - (*iter)->sensorPeriodPrv >
            ...
            float tempC =
    (*iter)->dallasTempObj->getTempC((*iter)->dallasTempAddr); if (tempC ==
    DEVICE_DISCONNECTED_C) { WS_DEBUG_PRINTLN("ERROR: Could not read temperature
    data, is the " "sensor disconnected?"); #ifdef USE_DISPLAY
    ...
    WS._buffer_outgoing, msgSz, 1)) return; WS_DEBUG_PRINTLN("PUBLISHED!");
    #ifdef USE_DISPLAY
              WS._ui_helper->add_text_to_terminal(buffer);
    #endif
     */
